...
class S3DISDataset(object):
    def __init__(self, cvfold, data_path,  bg_data_path=None):
        '''
        Args:
        ...
        self.fold_0 = ['table', 'window', 'column', 'beam', 'board', 'sofa']  # choose fold 0 as the tail classes [3, 4, 5, 7, 9, 11]
        self.fold_1 = ['wall', 'ceiling', 'floor', 'clutter', 'bookcase', 'door', 'chair']



        if cvfold == 0:
            self.test_classes = [self.type2class[i] for i in self.fold_0] # [3, 4, 5, 7, 9, 11]
        elif cvfold == 1:
        ...
        if bg_data_path is not None:
            ...
            self.pseudo_class2scans = self.get_pseudo_class2scans()
    ...
    def get_pseudo_class2scans(self):
        class2scans_file = os.path.join(self.data_path, self.bg_data_path, 'class2scans.pkl')
        if os.path.exists(class2scans_file):
            #load class2scans (dictionary)
            with open(class2scans_file, 'rb') as f:
                class2scans = pickle.load(f)
            ...
            min_ratio = .05  # to filter out scans with only rare labelled points
            min_pts = 100  # to filter out scans with only rare labelled points
            class2scans = {k:[] for k in range(len(self.test_classes))} # {0: [], 1:[], 2:[], ......., 12:[]}

            for file in sorted(glob.glob(os.path.join(self.data_path, self.bg_data_path, '*.npy'))):
                scan_name = os.path.basename(file)[:-4]
                data = np.load(file)
                labels = data[:,7].astype(np.int)
                if (labels+1).sum()==0: # labels are all -1
                    print(scan_name)
                    continue
                classes = np.unique(labels)
            ...
            print('==== class to scans mapping is done ====')
            for class_id in range(len(self.test_classes)):
                print('\t class_id: {0} | min_ratio: {1} | min_pts: {2} | class_name: {3} | num of scans: {4}'.format(
                          class_id,  min_ratio, min_pts, self.class2type[class_id], len(class2scans[class_id])))
...
if __name__ == '__main__':
    ...
    # dataset = S3DISDatasetPseudo(0, 'datasets/S3DIS/blocks_bs1_s1', 'base0')
    print('done')