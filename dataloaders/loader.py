...
def sample_pointcloud(data_path, num_point, pc_attribs, pc_augm, pc_augm_config, scan_name,
                      ...
                      use_all_classes=False, bg_data_path=None, return_ori_data=False, return_sample_xyz=False):
    '''
    Args:
        data_path:
        num_point:
    ...
    if return_sample_xyz:
        return ptcloud, groundtruth, pseudo_label, xyz_sample
...
class MyPretrainDataset(Dataset):
    def __init__(self, data_path, classes, class2scans, mode='train', num_point=4096, pc_attribs='xyz',
                       pc_augm=False, pc_augm_config=None, return_name=False, return_ori_data=False, bg_data_path=None):
    ...
    def __getitem__(self, index):
        ...
        # print(block_name)
        ptcloud, label, segment_label, ori_data = sample_pointcloud(self.data_path, self.num_point, self.pc_attribs, self.pc_augm,
                                                                    self.pc_augm_config, block_name, self.classes, random_sample=True, 
                                                                    bg_data_path=self.bg_data_path, return_ori_data=True)
        ...
        if self.return_ori_data:
            return torch.from_numpy(ptcloud.transpose().astype(np.float32)), torch.from_numpy(label.astype(np.int64)), \
                   torch.from_numpy(segment_label.astype(np.float32)), block_name, ori_data
        elif self.return_name:
            return torch.from_numpy(ptcloud.transpose().astype(np.float32)), torch.from_numpy(label.astype(np.int64)), \
                   torch.from_numpy(segment_label.astype(np.float32)), block_name
...
def sample_pointcloud_pseudo(data_path, num_point, pc_attribs, pc_augm, pc_augm_config, scan_name,
                                sampled_classes, sampled_class=0, support=False, random_sample=False, 
                                use_all_classes=False, bg_data_path=None, up_bnd=False):
    ...
    if bg_data_path is not None:
        if up_bnd:
            # print('i')
            pseudo_label = data[:,6]
            gt_bg_mask = groundtruth==0 # represent background
            pseudo_mask = pseudo_label>-1
            pseudo_mask = pseudo_mask & gt_bg_mask
            # [7, 5, 4, 3, 11, 9]
            pseudo_label[pseudo_label==7] = 10
            pseudo_label[pseudo_label==5] = 9
            pseudo_label[pseudo_label==4] = 8
            pseudo_label[pseudo_label==3] = 7
            pseudo_label[pseudo_label==11] = 12
            pseudo_label[pseudo_label==9] = 11
            groundtruth[pseudo_mask] = pseudo_label[pseudo_mask] + 1
        else:
            pseudo_label = data[:, 7]
            gt_bg_mask = groundtruth==0 # represent background
            pseudo_mask = pseudo_label>-1
            pseudo_mask = pseudo_mask & gt_bg_mask
            # [7, 5, 4, 3, 11, 9]
            pseudo_label[pseudo_label==0] = 10
            pseudo_label[pseudo_label==1] = 9
            pseudo_label[pseudo_label==2] = 8
            pseudo_label[pseudo_label==3] = 7
            pseudo_label[pseudo_label==4] = 12
            pseudo_label[pseudo_label==5] = 11
            groundtruth[pseudo_mask] = pseudo_label[pseudo_mask] + 1
    else:
        print('error!!!')
        # pseudo_label = np.zeros(data.shape[0], dtype=data.dtype)
...
class PseudoDataset(Dataset):
    def __init__(self, data_path, classes, class2scans, mode='train', num_point=4096, pc_attribs='xyz',
                       pc_augm=False, pc_augm_config=None, bg_data_path=None, up_bnd=False):
        ''' dataset of base classes
        Args:
            data_path:
            classes: make sure they are in order.
    ...
    def __getitem__(self, index):
        ...
        ptcloud, label = sample_pointcloud_pseudo(self.data_path, self.num_point, self.pc_attribs, self.pc_augm,
                                                                            self.pc_augm_config, block_name, self.classes, random_sample=True,
                                                                            bg_data_path=self.bg_data_path, up_bnd=self.up_bnd)
...
class NovelClusterDataset(Dataset):
    def __init__(self, data_path, classes, class2scans, mode='train', num_point=4096, pc_attribs='xyz',
                       pc_augm=False, pc_augm_config=None, return_name=False, return_ori_data=False, bg_data_path=None,
                       novel_classes=None):
        super(NovelClusterDataset).__init__()
        self.data_path = data_path # train dataset
        self.classes = classes # train classes name
        self.num_point = num_point # 2048
        self.pc_attribs = pc_attribs # xyzrgbXYZ
    ...
    def __getitem__(self, index):
        block_name = self.block_names[index]
        # print(block_name)
        data = np.load(os.path.join(self.data_path, 'data', '%s.npy' %block_name))
        valid_point_inds_lst = []
        for nc in self.novel_classes:
            valid_point_inds_lst.append(np.nonzero(data[:,6] == nc)[0])
        valid_point_inds = np.concatenate(valid_point_inds_lst)
        if valid_point_inds.shape[0]==0:
            return 0
        sampled_point_inds = np.random.choice(valid_point_inds, self.num_point, replace=(len(valid_point_inds) < self.num_point))
        data = data[sampled_point_inds]
        xyz = data[:, 0:3]
        rgb = data[:, 3:6]
        ...
        for i, label in enumerate(labels):
            if label in self.classes:
                groundtruth[i] = self.classes.index(label) # no reservation of 'bg' class
        assert groundtruth.max() <= max(self.classes)

        return torch.from_numpy(ptcloud.transpose().astype(np.float32)), torch.from_numpy(groundtruth.astype(np.int64)), block_name, data
        
        
class BgGtDataset(Dataset):
    def __init__(self, data_path, classes, class2scans, mode='train', num_point=4096, pc_attribs='xyz',
                       pc_augm=False, pc_augm_config=None, return_name=False, test_class_names=None):
        super(BgGtDataset).__init__()
        self.data_path = data_path # train dataset
        self.classes = classes # train classes name
        self.num_point = num_point # 2048
        self.pc_attribs = pc_attribs # xyzrgbXYZ
    ...
    def __getitem__(self, index):
        block_name = self.block_names[index]
        # print(block_name)
        ptcloud, label, segment_label = sample_pointcloud_testlabel(self.data_path, self.num_point, self.pc_attribs, self.pc_augm,
                                           self.pc_augm_config, block_name, self.classes, random_sample=True, test_class_names=self.test_class_names)
...
def sample_pointcloud_testlabel(data_path, num_point, pc_attribs, pc_augm, pc_augm_config, scan_name,
                      sampled_classes, sampled_class=0, support=False, random_sample=False, use_all_classes=False, test_class_names=None):
    sampled_classes = list(sampled_classes) # pre-train: train classes.  train: 2-way classes name
    data = np.load(os.path.join(data_path, 'data', '%s.npy' %scan_name))
    N = data.shape[0] #number of points in this scan (BLOACK)
    ...
    # get labels
    assert test_class_names is not None
    labels = data[:, 6].astype(np.int)
    groundtruth = np.zeros_like(labels) # labels that only availabel in the given classes
    py = np.zeros_like(labels)
    for i, label in enumerate(labels):
        if label in sampled_classes:
            groundtruth[i] = sampled_classes.index(label) + 1
        if label in test_class_names:
            py[i] = test_class_names.index(label) + 1
    assert groundtruth.max() <= max(sampled_classes)

    return ptcloud, groundtruth, py

class PseudoBgDataset(Dataset):
    def __init__(self, data_path, classes, class2scans, mode='train', num_point=4096, pc_attribs='xyz',
                       pc_augm=False, pc_augm_config=None, return_name=False, bg_data_path=None):
       
        super(PseudoBgDataset).__init__()
        self.data_path = data_path # train dataset
        self.classes = classes # train classes name
        self.num_point = num_point # 2048
        self.pc_attribs = pc_attribs # xyzrgbXYZ
    ...
    def __len__(self):
        return len(self.block_names)*10

    def __getitem__(self, index):
        idx, locus = divmod(index, 10)
        block_name = self.block_names[idx]
        data = np.load(os.path.join(self.data_path, self.bg_data_path, f'{locus}', '%s.npy' %block_name))
        xyz = data[:, 0:3]
        rgb = data[:, 3:6]
        xyz_min = np.amin(xyz, axis=0)
        xyz -= xyz_min
        ...
        ptcloud = []
        if 'xyz' in self.pc_attribs: ptcloud.append(xyz)
        if 'rgb' in self.pc_attribs: ptcloud.append(rgb) # already normalized
        if 'XYZ' in self.pc_attribs: ptcloud.append(XYZ)
        ptcloud = np.concatenate(ptcloud, axis=1) # (2048, 9)

        # get labels
        labels = data[:, 9].astype(np.int64)
        plabels = data[:, 10].astype(np.int64)

        return torch.from_numpy(ptcloud.transpose().astype(np.float32)), torch.from_numpy(labels.astype(np.int64)),  torch.from_numpy(plabels.astype(np.int64))

class PseudoPretrainDataset(Dataset):
    def __init__(self, data_path, classes, class2scans, mode='train', num_point=4096, pc_attribs='xyz',
                       pc_augm=False, pc_augm_config=None, return_name=False):
       
        super(PseudoPretrainDataset).__init__()
        self.data_path = data_path # train dataset
        self.classes = classes # train classes name
        self.num_point = num_point # 2048
        self.pc_attribs = pc_attribs # xyzrgbXYZ
    ...
    def __getitem__(self, index):
        idx, locus = divmod(index, 10)
        block_name = self.block_names[idx]
        data = np.load(os.path.join(self.data_path, 'c16v', f'{locus}', '%s.npy' %block_name))
        xyz = data[:, 0:3]
        rgb = data[:, 3:6]
        xyz_min = np.amin(xyz, axis=0)
        xyz -= xyz_min
        ...
        return torch.from_numpy(ptcloud.transpose().astype(np.float32)), torch.from_numpy(plabels.astype(np.int64))
...
class Testing_Dataset(Dataset):
    def __init__(self, data_path, class_names, learning_order, class2scans, mode='train', num_point=4096, pc_attribs='xyz',
                       pc_augm=False, pc_augm_config=None, visualize=False):
        ...
        print('[Testing Dataset] Mode: {0} | Num_blocks: {1}'.format(mode, len(self.block_names)))
    ...
    def create_static_testing_data(self):
        ...
        for i in range(len(block_list)):
            ...
            pcd, label, segment_label, xyz = sample_pointcloud(self.data_path, self.num_point, self.pc_attribs, self.pc_augm,
                                               self.pc_augm_config, block_name, self.classes, random_sample=True, use_all_classes=True,
                                               return_sample_xyz=True)
            # save in local dir
            ...
            os.makedirs(os.path.join(self.save_path, 'xyz'), exist_ok=True)


            with open(os.path.join(self.save_path, 'pcd', '{}.pkl'.format(block_name)), 'wb') as f:
                pickle.dump(pcd, f)  # (2048, 9)
            # save mask
            with open(os.path.join(self.save_path, 'label', '{}.pkl'.format(block_name)), 'wb') as f:
                pickle.dump(label, f)  # (2048,)
            # save mask
            with open(os.path.join(self.save_path, 'segment_label', '{}.pkl'.format(block_name)), 'wb') as f:
                pickle.dump(segment_label, f)  # (2048,)
            with open(os.path.join(self.save_path, 'xyz', '{}.pkl'.format(block_name)), 'wb') as f:
                pickle.dump(xyz, f)  # (2048,)
    ...
    def __getitem__(self, index):
        ...
        with open(os.path.join(self.save_path, 'xyz', pcd_name), 'rb') as f:
            xyz = pickle.load(f)
            
        if self.visualize:
            return torch.from_numpy(pcd.transpose().astype(np.float32)), torch.from_numpy(final_label.astype(np.int64)), \
                    torch.from_numpy(segment_label.astype(np.float32)), torch.from_numpy(xyz.astype(np.float32)), block_name
        else:
            return torch.from_numpy(pcd.transpose().astype(np.float32)), torch.from_numpy(final_label.astype(np.int64)), \
                    torch.from_numpy(segment_label.astype(np.float32))
