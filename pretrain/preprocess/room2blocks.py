...
if __name__ == '__main__':
    ...
    # s3dis
    parser.add_argument('--data_path', default='/home/jimtsai/GFS-3DSeg_GWs/datasets/S3DIS/scenes')
    parser.add_argument('--dataset', default='s3dis', metavar='bs', help='s3dis|scannet')

    # scannet
    # parser.add_argument('--data_path', default='/home/jimtsai/GFS-3DSeg_GWs/datasets/ScanNet/scenes')
    # parser.add_argument('--data_path', default='/home/jimtsai/fewshot/datasets/ScanNet/scenes')
    # parser.add_argument('--dataset', default='scannet', metavar='bs', help='s3dis|scannet')
    ...
    parser.add_argument('--exp', type=str, default='base0_mix')
    # parser.add_argument('--exp', type=str, default='pt_pcd_camz01_adapt_w140')
    ...
    if not args.pseudo:
        ...
        print('generate pseudo mask')
        SAVE_PATH = os.path.join(os.path.dirname(DATA_PATH), 'blocks_bs{0}_s{1}'.format(BLOCK_SIZE, STRIDE), args.exp)
        Path(SAVE_PATH).mkdir(parents=True,exist_ok=True)
        file_paths = glob.glob(os.path.join(DATA_PATH, args.exp, '*.npy'))
        print('{} scenes to be split...'.format(len(file_paths)))
